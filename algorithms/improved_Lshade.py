    ...
    def optimize(self):
        ...
        while self.evals < self.max_evals and NP >= self.pop_min:
            ...
            for i in range(NP):
                ...
                pmin = 2.0 / NP
                p = pmin + np.random.rand() * (0.2 - pmin)  # p ∈ [2/NP, 0.2]
                pbest_size = max(2, int(p * NP))
                top_indices = np.argsort(fitness)[:pbest_size]
                ...
                mutant = population[i] + f * (x_pbest - population[i]) + f * (a - b)
...
for i, FuncClass in enumerate(cec_functions, start=1):
    f = FuncClass(ndim=20)
    optimizer = LSHADE(f, max_evals=100_000, pop_init=300, pop_min=4, memory_size=10)
